...
class StoreAdminAdapter(
    ...
    private val onArtiklClickListener: onIzbrisiButtonCliked
) : RecyclerView.Adapter<StoreAdminAdapter.ArtiklViewHolder>() {


    class ArtiklViewHolder(val artiklView: View) : RecyclerView.ViewHolder(artiklView)

    override fun getItemCount(): Int {
        if (kupovni) return kupovniArtiklList.size
        else return rentabilniArtiklList.size
    }
    ...
    override fun onBindViewHolder(holder: ArtiklViewHolder, position: Int) {
        with(holder.artiklView) {
            if (kupovni) {
                ...
                artiklVrsta.text = "Grupa: " + kupovniArtiklList[position].artikl.idGrupa.toString()
                artiklCijena.text = "Cijena: " + kupovniArtiklList[position].cijena.toString()
                artiklIznajmljen.visibility = View.GONE
            } else {
                //Za rent:
                artiklNaziv.text = "Naziv: " + rentabilniArtiklList[position].naziv
                artiklOpis.text = "Opis: " + rentabilniArtiklList[position].opis
                artiklVrsta.text = "Grupa: " + rentabilniArtiklList[position].idGrupa.toString()
                artiklCijena.text =
                    "Cijena/dan: " + rentabilniArtiklList[position].cijena.toString()
                artiklIznajmljen.visibility = View.VISIBLE
            ...
            addToCartBtn.setOnClickListener {
                storeToPref(Constants.BASKET_ID, position)
                Toast.makeText(adapter, "Artikl dodan!", Toast.LENGTH_LONG).show()
            }

            deleteItemBtn.setOnClickListener {
                onArtiklClickListener.onIzbrisiClicked(kupovniArtiklList[position].sifraArtikl!!.toInt())
            }
    ...
    interface onIzbrisiButtonCliked {
        fun onIzbrisiClicked(artiklId: Int)
    }

    private fun storeToPref(where: String, pos: Int) {
